...
Для видео-разговора будем использовать каналы связи, которые будут пересоздаваться каждый час.
...
По официальной документации Nginx его RPS на 8 ядер CPU и запросами не более 100КБ выдерживает 91,640 RPS. Также для поддержки видео-разговора, где сетевой трафик составляет 4.166.667 Гб/час = 1157,4 Гб/сек, будем использовать быструю пропускную способность сети. Для этого минимум будет необходимо всего 24 сервера. На каждый ДЦ рассположим по 3 активных сервера и по 1 резервному на случай, если что-то упадет.
Параметр | Значение
------------ | -------------
CPU | 8
Network | 400Gbit/s ([такое существует](https://docs.microsoft.com/sql/t-sql/statements/set-transaction-isolation-level-transact-sql))
RAM | 16Gb
SSD | 512Gb

### Пользователи
##### Сервис
RPS на сервис сообщений будет составлять сумме RPS на смену аватарки и смене имени пользователя, а также авторизации. Это будет около 300-500 RPS, учитывая также запросы с других сервисов. Сетевой трафик - 155Мб/сек. Возьмем 2 сервера на каждый ДЦ, один из которых будет резервным.
Параметр | Значение
------------ | -------------
CPU | 4
Network | 5Gbit/s
...
##### PostgresSQL
200.000 Гб - размер хранения всех пользователей. На каждом ДЦ будем использовать 2 сервера. Один из которых будет master, а другой slave.
...
Для каждого ДЦ используем 2 сервера, один из которых резервный.
Параметр | Значение
------------ | -------------
CPU | 8
Network | 5Gbit/s
...
##### PostgresSQL
143.942 Гб за год размер хранения всех сообщений. Каждый год на каждый ДЦ будем прибавлять по 2 сервера - master-slave. На master будет осуществляться запись и чтение текущих сообщений, а на slave будет производиться чтение архивированных чатов
...
##### Сервис
RPS на сервис будет составлять 771RPS - это немного, но нам необходимы жирные каналы связи для поддержки сетевого трафика 1157,4 Гб/сек. Для такого сетевого трафика и RPS минимум подойдет 56 сервера с заданными конфигурациями, с учетом микширования и параллельной загрузки на Amazon. На каждый ДЦ разместим 5 серверов и 2 резервных сервера для повышения стабильности работы приложения.
Параметр | Значение
------------ | -------------
CPU | 8
Network | 60Gbit/s
...
##### PostgresSQL
Максимальными RPS для PostgreSql возьмем - 5000RPS. Тогда для 771RPS будет необходимо 2 сервера на каждый ДЦ(master-slave) с небольшим объемом данных, потому что мы храним только мету информацию видео-звонков. Запись будем производить на master, а чтение со slave.
...
### Сессия
##### Сервис:
Внешний RPS авторизации составляет 2.314. Также к сервису авторизации обращаюся все остальные сервисы для проверки авторизации, тогда добавляется 9.257 RPS. Всего 11.571 RPS. Тогда на сервисе сессий нам понадобиться 3 сервера в каждом ДЦ, один из которых резервный.
Параметр | Значение
------------ | -------------
CPU | 4
RAM | 8Gb
...
##### Tarantool:
Tarantool хранит свои данные в оперативной памяти, нам нужно еще подсчитать. Одна запись ключ-значение будет занимать примерно 40байт (int64 + sha_256). Тогда для 200.000.000 пользователей это займет 7.4 Гб. Значение небольшое - хватит 2 сервера на каждый ДЦ, один из которых резервный.
