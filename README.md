...
По официальной документации Nginx его RPS на 8 ядер CPU и запросами не более 100КБ выдерживает 91,640 RPS. RPS видео-разговоров + аудио - около 2.500.000 RPS. Поэтому будет необходимо 28 серверов всего разделенных по ДЦ. Также для поддержки видео-разговора, где сетевой трафик составляет 4.166.667 Гб/час = 1157,4 Гб/сек будем использовать быструю пропускную способность сети.
Параметр | Значение
------------ | -------------
CPU | 8
...
##### Сервис
RPS на сервис будет составлять 771RPS - это немного, но нам необходимы жирные каналы связи для поддержки сетевого трафика 1157,4 Гб/сек. Для такого сетевого трафика и RPS подойдет 28 сервера с заданными конфигурациями.
Параметр | Значение
------------ | -------------
CPU | 8
Network | 60Gbit/s
...
##### PostgresSQL
Максимальными RPS для PostgreSql возьмем - 5000RPS. Тогда для 771RPS будет необходимо 2 сервера с небольшим объемом данных, потому что мы храним только мету информацию видео-звонков.
...
##### Tarantool:
Tarantool хранит свои данные в оперативной памяти, нам нужно еще подсчитать. Одна запись ключ-значение будет занимать примерно 40байт (int64 + sha_256). Тогда для 200.000.000 пользователей это займет 7.4 Гб. Значение небольшое - хватит одного сервера.
