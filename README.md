...
Всего расположим 12 ДЦ, каждый из которых будет расчитан на 10% пользователей.
...
### Пользователи
##### Сервис
RPS на сервис сообщений будет составлять сумме RPS на смену аватарки и смене имени пользователя, а также авторизации. Это будет около 300-500 RPS, учитывая также запросы с других сервисов.
Параметр | Значение
------------ | -------------
CPU | 4
RAM | 16Gb
...
##### PostgresSQL
200.000 Гб - размер хранения всех пользователей. Будем использовать два сервера со след. конфигурациями
...
### Сообщения
##### Сервис
RPS на сервис пользователей будет составлять 2.314 RPS. Для такого сетевого трафика подойдет 2 сервера.
Параметр | Значение
------------ | -------------
CPU | 8
RAM | 32Gb
...
##### Сервис
RPS на сервис будет составлять 6.558 * 60 RPS = 393.480 RPS, если отправка видео-контента будет происходить каждую секунду. И сетевой трафик около 0.77 Tb/сек. Для такого сетевого трафика подойдет 56 сервера.
Параметр | Значение
------------ | -------------
CPU | 8
Network | 60Gbit/s
...
##### PostgresSQL
Для мета информации разговоров хватит 1 сервера
Параметр | Значение
------------ | -------------
CPU | 8
RAM | 16Gb
...
### Сессия
##### Сервис:
Внешний RPS авторизации составляет 2.314. Для него потребуются максимум 2 сервера. Также к сервису авторизации обращаюся все остальные сервисы для проверки авторизации для тогда добавляется 9.257 RPS. Всего 11.571 RPS. Тогда на серсив сессий нам понадобиться 2 сервера.
Параметр | Значение
------------ | -------------
CPU | 4
RAM | 8Gb
